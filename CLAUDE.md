...
Phases 0–4 are done and pushed. Phase 5 (Pokémon detail page) is in progress: the "core" slice (`PokemonHeader`/`BaseStats`/`AbilitiesList`/`TypeEffectiveness`) is done and working end-to-end; `EvolutionChain` (Swiper) and `MovesetTable`/`MoveTooltip` (needs `MoveApiService`, filtering, `@defer`) are still outstanding — paused here to check in with the user before that larger remaining chunk.
...
5. ⬅️ **next up** — Pokémon detail page, internally in this order: `PokemonHeader` → `BaseStats` → `AbilitiesList` → `TypeEffectiveness` → `EvolutionChain` → `MovesetTable`/`MoveTooltip`
